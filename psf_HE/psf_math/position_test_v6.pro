...
function get_z_zerr, a0=a0, b0=b0, array_x=array_x, array_y=array_y, array_psf=array_psf
    ...
    dex_array = sort_dis[0]
    ; dis_array = dis_all[dex_array]
	; dis_array = transpose(ret_array[0,*])
	; dex_array = transpose(ret_array[1,*])
    calc_z= array_psf[dex_array]
    return, calc_z
end

function get_psf, x=x, y=y, data_x=data_x, data_y=data_y, data_z=data_z, instru=instru, BoxID=BoxID
	...
	for i=0L, n-1 do begin
    	...
    	calc_z[i] = get_z_zerr(a0=x[i], b0=y[i], array_x=data_x, array_y=data_y, array_psf=data_z)
	endfor
	psf_value = calc_z
...
function modulate_curve_me_psf_modify, RA=RA, DEC=DEC, Flux=Flux, Qt1=Qt1, Qt2=Qt2, Qt3=Qt3, BoxID=BoxID, data_x=data_x, data_y=data_y, data_z=data_z
	...
	psf_value = get_psf(x=alpha_sf,y=beta_sf, data_x=data_x, data_y=data_y, data_z=data_z,instru=instru,BoxID=BoxID)
...
pro position_test_v6, energy_LE=energy_LE, energy_ME=energy_ME, burst_dex=burst_dex, print_result=print_result, use_LE=use_LE, use_ME=use_ME,n_step=n_step,n_burn=n_burn
	...
	if dex_src_le[0] ne -1 and dex_src_me[0] ne -1 then begin
        ...
        instru = 'ME'
        BoxID = 0
        data_x0 = transpose(read_data(STRCOMPRESS(instru+'_psf_alpha_box'+strtrim(BoxID)+'.txt', /REMOVE_ALL)))
        data_y0 = transpose(read_data(STRCOMPRESS(instru+'_psf_beta_box'+strtrim(BoxID)+'.txt', /REMOVE_ALL)))
        data_z0 = transpose(read_data(STRCOMPRESS(instru+'_psf_value_box'+strtrim(BoxID)+'.txt', /REMOVE_ALL)))

        BoxID = 1
        data_x1 = transpose(read_data(STRCOMPRESS(instru+'_psf_alpha_box'+strtrim(BoxID)+'.txt', /REMOVE_ALL)))
        data_y1 = transpose(read_data(STRCOMPRESS(instru+'_psf_beta_box'+strtrim(BoxID)+'.txt', /REMOVE_ALL)))
        data_z1 = transpose(read_data(STRCOMPRESS(instru+'_psf_value_box'+strtrim(BoxID)+'.txt', /REMOVE_ALL)))

        BoxID = 2
        data_x2 = transpose(read_data(STRCOMPRESS(instru+'_psf_alpha_box'+strtrim(BoxID)+'.txt', /REMOVE_ALL)))
        data_y2 = transpose(read_data(STRCOMPRESS(instru+'_psf_beta_box'+strtrim(BoxID)+'.txt', /REMOVE_ALL)))
        data_z2 = transpose(read_data(STRCOMPRESS(instru+'_psf_value_box'+strtrim(BoxID)+'.txt', /REMOVE_ALL)))
		;------------------------------------ Fit (MCMC) ------------------------------------
		;n_step=1d+06
		...
		for ii=0L, n_step-1 do begin
			...
			burst_me_0_tem = modulate_curve_me_psf_modify(RA=RA, DEC=DEC, Flux=Flux_ME, Qt1=Q1_me, Qt2=Q2_me, Qt3=Q3_me, BoxID=0, data_x=data_x0, data_y=data_y0, data_z=data_z0);, DL=DL_ME_0, euler=euler_ME_0, par=par_ME_0)
			burst_me_1_tem = modulate_curve_me_psf_modify(RA=RA, DEC=DEC, Flux=Flux_ME, Qt1=Q1_me, Qt2=Q2_me, Qt3=Q3_me, BoxID=1, data_x=data_x1, data_y=data_y1, data_z=data_z1);, DL=DL_ME_1, euler=euler_ME_1, par=par_ME_1)
			burst_me_2_tem = modulate_curve_me_psf_modify(RA=RA, DEC=DEC, Flux=Flux_ME, Qt1=Q1_me, Qt2=Q2_me, Qt3=Q3_me, BoxID=2, data_x=data_x2, data_y=data_y2, data_z=data_z2);, DL=DL_ME_2, euler=euler_ME_2, par=par_ME_2)
