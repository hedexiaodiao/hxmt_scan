...
def psf(Astr):
    paras = np.loadtxt('./Parabolic_%s_%s_220522.txt' % (instr, str(Astr)))
    ...
    def stand_psfmodel(delta_alfa0, delta_beta0, box_dex=0):
        ...
        factor = factor / (np.sqrt(
            np.tan(delta_alfa / 180.0 * PI) * np.tan(delta_alfa / 180.0 * PI) + np.tan(
                delta_beta / 180.0 * PI) * np.tan(
                delta_beta / 180 * PI) + 1))
        return factor
    ...
    def correct_psfmodel(delta_alfa0, delta_beta0, box_dex=0):
        ...
        x, y, z = ab2xyz(np.deg2rad(delta_alfa0),
                         np.deg2rad(delta_beta0))  # testroll.sph2cart(ra * pi / 180, dec * pi / 180, 1)
        xyz = [x, y, z]
    ...
    def correct_psfmodel2(delta_alfa0, delta_beta0, box_dex=0):
        PI = 3.14159265358979323846
        # flag = parameter[0]
        roll = 0  # parameter[1] / 180.0 * PI
        roll_indx = box_dex  # int(-parameter[1] / 60 + 1)
