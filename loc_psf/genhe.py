...
ehkfile = outpath + 'ehk/%sehk.fits'%obsid
...
#if not os.path.exists(ehkfile):
#    comd = "; hxmtehkgen orbfile="+ basefile["Orbit"] +" attfile="+ basefile["ATT"] +" outfile="+ ehkfile +" saafile="+ saafile +" step_sec=0.25  clobber=yes"
#    os.system(cmdinit + comd)

#basefile['EHK'] = ehkfile

if not os.path.exists(pifile):
    cmd +=' ; hepical evtfile="%s" outfile="%s" clobber=yes'%(basefile['EVT'],pifile)

if not os.path.exists(gtifile):
    cmd += ' ; hegtigen hvfile="%s" tempfile="%s" pmfile="%s" outfile="%s" ehkfile="%s" defaultexpr=NONE expr="ELV>0&&SAA_FLAG==0" pmexpr="" clobber=yes history=yes'%(basefile['HV'], basefile['TH'], basefile['PM'],gtifile, basefile['EHK'])
...
xml = ElementTree()
xml.parse(sys.argv[2])
minpi = [int(i) for i in xml.find('PARAMETER/MinPiPara').text.split(',')]
maxpi = [int(i) for i in xml.find('PARAMETER/MaxPiPara').text.split(',')]
...
cmd+=' ; python hebkgmap_id14_202005.py lc %s %s %s %s %s %s %s %s %s'%(screenfile, basefile['EHK'], gtifile,basefile['DTime'],lcnametxt,minpi[i], maxpi[i], bkgfile,sys.argv[2])
os.system(cmdinit + cmd)
for i in range(18):
    if i==16:continue;
