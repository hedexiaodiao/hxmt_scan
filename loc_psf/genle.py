...
if not os.path.exists(gtifile):
    ...
    cmd +=' ;legtigen evtfile=%s tempfile="%s" instatusfile="%s" ehkfile="%s" outfile="%s" defaultexpr=NONE expr="ELV>5" clobber=yes history=yes'%(basefile['EVT'],basefile['TH'],basefile['InsStat'], basefile['EHK'], gtifile)
    cmd +=' ;legti %s %s %s'%(grdfile, gtifile, newgtifile)
    #os.system()

if not os.path.exists(screenfile):
    cmd +=' ;lescreen evtfile=%s gtifile=%s outfile=%s userdetid="0-95"'%(grdfile,gtifile,screenfile)
    #os.system()
...
cmd = ';lelcgen evtfile=%s outfile=%s binsize=1 starttime=%s stoptime=%s minPI=%s maxPI=%s userdetid="0,2,3,4,6,7,8,9,10,12,14,20,22,23,24,25,26,28,30;32,34,35,36,38,39,40,41,42,44,46,52,54,55,56,57,58,60,61,62;64,66,67,68,70,71,72,73,74,76,78,84,86,88,89,90,92,93,94" eventtype=1'%(screenfile,lcfile,startm,stoptm,minpi,maxpi)
