...
2019-10-31:
The correctoin for the removed pixel 54.
...
Add the background lightcurves of three boxes
...
print( "*********************************************************" )
print( "HXMTDAS version ", SVer," HXMT background for Insight-HXMT/LE, ver-", BVer )
...
def write_bkgspec(fname,channel,counts,spec_err,expo,hdr_ext):
    spec_qua= np.zeros(1536)
    spec_grp= np.ones(1536)
...
try:
    ...
    print(tmpstr[0:tmppos]," is empty!")
    sys.exit()

'''Read Simple model'''

specmodelname = REFPATH+'Spec_Model.txt'
print(specmodelname)

try:
    simbkgmod_list= np.loadtxt(specmodelname)
    simbkgmod_map = simbkgmod_list[:,2]
    simbkgmod_maperr = simbkgmod_list[:,3]
except IOError:
    print(specmodelname," is empty!")
    sys.exit()

'''Read Simple binned model'''
specmodelname_192 = REFPATH+'Spec_Model_192.txt'
print(specmodelname_192)
try:
    simbkgmod_list= np.loadtxt(specmodelname_192)
    simbkgmod_map_192 = simbkgmod_list[:,2]
    simbkgmod_maperr_192 = simbkgmod_list[:,3]
except IOError:
    print(specmodelname_192," is empty!")
    sys.exit()

#fac_all = 3.1554279+3.4639846+2.8486108
'''For Box200000'''
#fac_all = 3.1554279
#fac_all = 3.4639846
fac_all = 2.8486108
...
#cxbmapfile=REFPATH+'CXB_LE_BA_rebuild_1536_smooth.txt'
#print(cxbmapfile)

'''For Box20000000000000'''

#cxbmapfile='CXB_LE_B0_rebuild_1536_smooth.txt'
#print(cxbmapfile)

#cxbmapfile='CXB_LE_B1_rebuild_1536_smooth.txt'
#print(cxbmapfile)
cxbmapfile='CXB_LE_B2_rebuild_1536_smooth.txt'
print(cxbmapfile)

'''For Box20000000000000'''
try:
    cxbkg_list= np.loadtxt(cxbmapfile)
    cxbkg_map = cxbkg_list[:,1]
except IOError:
    print(cxbmapfile," is empty!")
    sys.exit()

'''Read Galaxy X-ray Background'''
gxmapfile=REFPATH+'LE_gxmap_v2.6.fits'
try:
    gxbkg_list= pf.open(gxmapfile)
...
MJDREF_HXMT = 55927.0 + 66.184/86400


ts0 = (58652.65648361-MJDREF_HXMT)*86400.0
coe_pixel = 1.0
if t00 >= ts0:
    coe_pixel = 57./58.0

#print("Pixel correction: ", coe_pixel)
...
if sp_lc_select == 'spec':
    ...
    spec_cnt = spec_cnt2*coe_pixel
    spec_err = spec_err2*coe_pixel
...
if sp_lc_select == 'lc':
    ...
    lc_bkg = np.interp(lc_time,tmptime_yf,tmprate_yf2)*coe_pixel
    lc_err = np.interp(lc_time,tmptime_yf,lc_bkg_err[norm_in])*coe_pixel
    #plt.figure()
    #plt.plot(lc_tim_map[norm_in]-lc_tim_map[0],lc_bkg_map[norm_in])
    #plt.plot(lc_tim_map[norm_in]-lc_tim_map[0],tmprate_yf2)
    #plt.show()
    outname = outnam + '.lc'
    print("The background light curve: ",outname)
    write_lcurve(outname,lc_time,lc_bkg,lc_err,lc_hdr)
    ''' For each box '''
    #rr_all = 3.1548740 + 3.4587939 + 2.8518058
    #r0_box0 = 3.1548740/rr_all;
    #r0_box1 = 3.4587939/rr_all;
    #r0_box2 = 2.8518058/rr_all;
    #rr_all = 19+20+19
    #r0_box0 = 19/rr_all;
    #r0_box1 = 20/rr_all;
    #r0_box2 = 19/rr_all;
    ...
    #write_lcurve(outname,lc_time,lc_bkg*r0_box0,lc_err*r0_box0,lc_hdr)
    #outname = outnam + '_box1.lc'
    #write_lcurve(outname,lc_time,lc_bkg*r0_box1,lc_err*r0_box1,lc_hdr)
    #outname = outnam + '_box2.lc'
    #write_lcurve(outname,lc_time,lc_bkg*r0_box2,lc_err*r0_box2,lc_hdr)
print("Finish.")
