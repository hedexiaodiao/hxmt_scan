...
2020-05-13
rr0 =   0.267460781848635
rr1 =   0.362947205066505
rr2 =   0.369592013084860
...
print( "*********************************************************" )
print( "HXMTDAS version ", SVer," HXMT background for Insight-HXMT/ME, ver-", BVer )
    ...
    for i in xrange(len_arg):
        ...
        '''if (len(sys.argv)==13):
            if argname == 'newgti':
                ...
                sysflag = sysflag + 1
        '''
    if((sysflag>0)&(sysflag<10)):
        raise IOError("Error input argument, RUN 'mebkgmap -h' for help")
        sys.exit()
    if ((sysflag==0)&(len_arg>=11)):
...
def write_bkgspec(fname,channel,counts,expo,hdr_ext):
    ...
    spec_col3 = pf.Column(name='STAT_ERR', format='D',array=np.sqrt(counts)/expo)
...
def write_bkgspec2(fname,channel,counts,counts_err,expo,hdr_ext):
    spec_qua= np.zeros(1024)
    spec_grp= np.ones(1024)
    spec_col1 = pf.Column(name='CHANNEL', format='J', array=channel)
    #spec_col2 = pf.Column(name='COUNTS', format='D', array=counts)
    spec_col2 = pf.Column(name='RATE', format='D',unit='counts/s', array=counts/expo)
    spec_col3 = pf.Column(name='STAT_ERR', format='D',array=counts_err/expo)
    spec_col4 = pf.Column(name='QUALITY', format='I', array=spec_qua)
    spec_col5 = pf.Column(name='GROUPING', format='I', array=spec_grp)
    cols = pf.ColDefs([spec_col1, spec_col2, spec_col3, spec_col4, spec_col5])
    hdr = pf.Header()
...
#print '#######################'
#print(bkgmap_start_index,bkgmap_stop_index)
#print(bkgmap_stop_index-bkgmap_start_index+1)
#print(bkgmap_stop_time-bkgmap_start_time)

bkgspec_bld_bkgmap = np.zeros((bkgmap_num,medetchans),dtype='float')
bkgspec_blderr_bkgmap = np.zeros((bkgmap_num,medetchans),dtype='float')

bkgspec_all_bkgmap = np.zeros((bkgmap_num,medetchans+3),dtype='float')
bkgspec_allerr_bkgmap = np.zeros((bkgmap_num,medetchans+3),dtype='float')

bkgspec_err_bkgmap = np.zeros((bkgmap_num,medetchans+3),dtype='float')

bkgspec_LON_bkgmap = np.zeros((bkgmap_num),dtype='float')
bkgspec_LAT_bkgmap = np.zeros((bkgmap_num),dtype='float')
bkgspec_EXP_bkgmap = np.zeros((bkgmap_num),dtype='float')
...
for ii in xrange(0,mefpganumber):
    for jj in xrange(0,bkgmap_num):
        ...
        tflag = np.zeros(dtime_num,dtype='int')
        tindex = np.zeros(dtime_num,dtype='int')
        time_gtiflag(dtime_time,t0-1,t1,tflag)
        tmpdt_num = flag_selection(tflag,tindex)
        tmpdeadtime = dtime_arr[tindex[0:tmpdt_num],ii]
        #if (ii==1):
        #    plt.figure()
        #    plt.plot(tmpdeadtime)
        #    plt.show()
        #    print(np.sum(tmpdeadtime),np.size(tmpdeadtime),tmpdt_num)
        if np.size(tmpdeadtime)==0:
            bkgmap_dtc[ii,jj] = 0
            continue
        bkgmap_dtc[ii,jj] = np.mean(tmpdeadtime)
...
try:
    ...
    evt_detid2  = evt_tab.field('Det_ID')
    evt_asic  = evt_tab.field('ASIC_ID')
    evt_fpga  = evt_tab.field('FPGA_ID')
    evt_pixel  = evt_tab.field('PIXEL_ID')
...
bld_expo_arr = np.zeros((bkgmap_num,1))
blddetid_index = np.where(((evt_detid == 10)|(evt_detid == 28)|(evt_detid == 46)))
#blddetid_index = np.where(((evt_detid == 46)) & (evt_grade == 0))
...
evt_grade = evt_grade[blddetid_index]
evt_detid2 = evt_detid2[blddetid_index]
evt_pixel  = evt_pixel[blddetid_index]
evt_detid  = evt_detid[blddetid_index]

#pp = evt_detid*32 + evt_pixel
#ID_ID     = np.unique(pp)
#ID_BLD_Num= np.size(ID_ID)
#print("NUmer 1:",np.size(evt_detid[np.where(evt_detid == 10)]))
#print("NUmer 1:",np.size(evt_detid[np.where(evt_detid == 28)]))
#print("NUmer 1:",np.size(evt_detid[np.where(evt_detid == 46)]))
#print("Blind pixel number:",ID_BLD_Num)
...
for jj in xrange(0,bkgmap_num):
    ...
    print '++++++++',t0,t1,evt_time
    tmpid  = evt_detid[ttindex]
    tmpcha = evt_cha[ttindex]
    #tmpspec,bins = np.histogram(tmpcha,bins=cha_ran,range=[0,medetchans])#@
    #tmpbkgmap_dtc= (bkgmap_dtc[1,jj]+bkgmap_dtc[4,jj]+bkgmap_dtc[7,jj])/3
    ttindex0 = np.where(tmpid == 10)
    ttindex1 = np.where(tmpid == 28)
    ttindex2 = np.where(tmpid == 46)
    tmpcha0 = tmpcha[ttindex0]
    tmpcha1 = tmpcha[ttindex1]
    tmpcha2 = tmpcha[ttindex2]
    tmpspec0,bins = np.histogram(tmpcha0,bins=cha_ran,range=[0,medetchans])#@
    tmpspec1,bins = np.histogram(tmpcha1,bins=cha_ran,range=[0,medetchans])#@
    tmpspec2,bins = np.histogram(tmpcha2,bins=cha_ran,range=[0,medetchans])#@
    #print((bkgmap_dtc[1,jj],bkgmap_dtc[4,jj],bkgmap_dtc[7,jj]))
    #print(t0,t1)
    #plt.figure()
    #plt.plot(tmpspec0)
    #plt.plot(tmpspec1)
    #plt.plot(tmpspec2)
    #plt.show()
    #print '********',tmpspec0,tmpspec1,tmpspec2,bkgmap_dtc[:,jj]
    bld_spec_arr[jj,0:medetchans] = tmpspec0/(1-bkgmap_dtc[1,jj]) + tmpspec1/(1-bkgmap_dtc[4,jj])+ tmpspec2/(1-bkgmap_dtc[7,jj])
    totalexpo_bld = totalexpo_bld + bkgmap_arr_expo[jj]
    bld_expo_arr[jj] = bkgmap_arr_expo[jj]
    #print((t1-t0) + 1)
    #plt.figure()
...
srcmapname=REFPATH+'/ME_bkgmap_201907.fits'
srcmapname='ME_bkgmap_201912.fits'

try:
    srclist = pf.open(srcmapname)
    srcmap_tab = srclist[1].data
    srcmap_IN  = srcmap_tab.field(0)
    srcmap_LAT = srcmap_tab.field(1)
    srcmap_LON = srcmap_tab.field(2)
    srcmap_BKG = srcmap_tab.field(3)
except IOError:
    print(srcmapname," is empty!")
    sys.exit()
...
for ii in xrange(0,bkgmap_num):
    ...
    tmpspec  = srcmap_BKG[tmpindex,0:4096]
    tmpexpo = bkgmap_arr_expo[ii]
    bkgspec_LON_bkgmap[ii] = srcmap_LON[tmpindex]
    bkgspec_LAT_bkgmap[ii] = srcmap_LAT[tmpindex]

    if(np.sum(tmpspec[tin1:tin2]) == -1024):
        bkgspec_bld_bkgmap[ii,0:1024] = tmpspec[tin1:tin2]*0
        bkgspec_blderr_bkgmap[ii,0:1024] = tmpspec[tin3:tin4]*0
        totalexpo_map = totalexpo_map + 0
    if(np.sum(tmpspec[tin1:tin2]) >= 0):
        bkgspec_bld_bkgmap[ii,0:1024] = tmpspec[tin1:tin2]*tmpexpo
        print(bkgspec_blderr_bkgmap.shape,tmpspec.shape,tin3,tin4)
        print bkgspec_blderr_bkgmap[ii,0:1024].shape,(tmpspec[tin3:tin4]*tmpexpo).shape,tmpspec[tin3:tin4],tmpexpo
        bkgspec_blderr_bkgmap[ii,0:1024] = tmpspec[tin3:tin4]*tmpexpo
        totalexpo_map = totalexpo_map + tmpexpo
        #print(tmpexpo)
        #plt.figure()
        #plt.plot(tmpspec[tin1:tin2])
        #plt.show()
...
rr_arr = np.zeros(bkgmap_num)
rrerr_arr = np.zeros(bkgmap_num)
#np.savetxt('mod_test.txt',bkgspec_bld_bkgmap)
totalexpo_map = 0
for ii in xrange(0,bkgmap_num):
    tspec_bld = bld_spec_arr[ii,0:1024]
    tspec_mod = bkgspec_bld_bkgmap[ii,0:1024]
    tspec_moderr = bkgspec_blderr_bkgmap[ii,0:1024]
    if np.sum(tspec_mod[100:700])==0.0:
        rr_arr[ii]    = 1
        rrerr_arr[ii] = 1
    if np.sum(tspec_mod[100:700])>0.0:
        r0 = np.sum(tspec_bld[100:700])
        r1 = np.sum(tspec_mod[100:700])
        #print '----',r0,r1
        tmprr = r0/r1
        r0err = np.sqrt(np.sum(tspec_bld[100:700]))/r0
        r1err = np.sqrt(np.sum(tspec_moderr[100:700]*tspec_moderr[100:700]))/r1
        rr_arr[ii] = np.mean(tmprr)
        rrerr_arr[ii] = rr_arr[ii]*np.sqrt(r0err*r0err + r1err*r1err)
    #print(bkgspec_LAT_bkgmap[ii],bkgspec_LON_bkgmap[ii],bkgmap_arr_expo[ii],rr_arr[ii],bld_expo_arr[ii])
    '''
    plt.figure()
    plt.plot(tspec_bld)
    plt.plot(tspec_mod)
...
rr0_box0 = 0.267460781848635
rr0_box1 = 0.362947205066505
rr0_box2 = 0.369592013084860
...
if (len(baddetfile)>0):
    ...
    #print('New bad pixel list: ',badpixel_new)
    ...
    print(new_num,'***********************************************************')
    tindex5 = tindex5[0:new_num]
    print(tindex5,pixel_fac_val.sum())
    pixel_fac_val_new = pixel_fac_val[tindex5]
    ...
    '''For three det boxes'''
    #print tindex5.shape
    in1_box0 = np.where((tindex5>=0)&(tindex5<576))
    in1_box1 = np.where((tindex5>=576)&(tindex5<1152))
    in1_box2 = np.where((tindex5>=1152)&(tindex5<1728))
    in1_box0 = np.in1d(tindex5,range(256)+range(352,576))
    in1_box1 = np.in1d(tindex5,range(576,832)+range(928,1152))
    in1_box2 = np.in1d(tindex5,range(1152,1408)+range(1504,1728))
    fac_box0 = np.sum(pixel_fac_val[tindex5[in1_box0]])
    fac_box1 = np.sum(pixel_fac_val[tindex5[in1_box1]])
    fac_box2 = np.sum(pixel_fac_val[tindex5[in1_box2]])
    print(fac_box0,fac_box1,fac_box2)
    print(480./fac_box0,480./fac_box1,480./fac_box2)
    print(np.intersect1d(pixel_fac_ID[tindex5[in1_box0]],pixel_fac_ID[pixel_fac_val!=0.]),np.intersect1d(pixel_fac_ID[tindex5[in1_box0]],pixel_fac_ID[pixel_fac_val!=0.]).shape)
    #print(pixel_fac_ID[tindex5[in1_box0]],'--------------------')
    rr0_box0 = fac_box0/(fac_box0+fac_box1+fac_box2)*1.172
    rr0_box1 = fac_box1/(fac_box0+fac_box1+fac_box2)*0.863
    rr0_box2 = fac_box2/(fac_box0+fac_box1+fac_box2)*0.996
    print(rr0_box0,rr0_box1,rr0_box2,'----------------------------------------')
    print(fac_box0+fac_box1+fac_box2,badpixel_cor)
...
for ii in xrange(0,bkgmap_num):
    ...
    bkgspec_allerr_bkgmap[ii,0] = 0
    bkgspec_allerr_bkgmap[ii,1] = (bkgmap_start_time[ii]+bkgmap_start_time[ii])/2.

    if(np.sum(tmpspec[tin1:tin2]) == -1024):
        bkgspec_all_bkgmap[ii,0:1024]    = tmpspec[tin1:tin2]*0
        bkgspec_all_bkgmap[ii,2] = 0
        bkgspec_allerr_bkgmap[ii,0:1024] = tmpspec[tin1:tin2]*0
        bkgspec_allerr_bkgmap[ii,2] = 0
    if(np.sum(tmpspec[tin1:tin2]) >= 0):
        bkgspec_all_bkgmap[ii,2] = bkgmap_arr_expo[ii]
        #print '::::',tmpspec[tin1:tin2],bkgmap_arr_expo[ii],rr_arr[ii]
        bkgspec_all_bkgmap[ii,3:1027] = tmpspec[tin1:tin2]*bkgmap_arr_expo[ii]*rr_arr[ii]
        bkgspec_all_bkgmap[ii,753:1027] = 0
        bkgspec_all_bkgmap[ii,3:53]     = 0
        bkgspec_allerr_bkgmap[ii,2] = bkgmap_arr_expo[ii]
        bkgspec_allerr_bkgmap[ii,3:1027] = bkgmap_arr_expo[ii]*np.sqrt((tmpspec[tin3:tin4]*rr_arr[ii])**2 + (tmpspec[tin1:tin2]*rrerr_arr[ii])**2)
...
for ii in xrange(0,bkgmap_num):
    ...
    #print(tmpexpo,np.sum(spec_cnt2))
    if tmpexpo==0:
        rate_bkgmod[ii] = 0
    if tmpexpo>0:
        #print(ii,tmpexpo)
...
#print bkgspec_all_bkgmap
tmpindex1 = np.where(rate_bkgmod > 0)
tmpindex2 = np.where(rate_bkgmod == 0)
#print time_bkgmod[tmpindex2],time_bkgmod[tmpindex1],rate_bkgmod[tmpindex1]
rate_bkgmod[tmpindex2] = np.interp(time_bkgmod[tmpindex2],time_bkgmod[tmpindex1],rate_bkgmod[tmpindex1])
raterr_bkgmod[tmpindex2] = np.interp(time_bkgmod[tmpindex2],time_bkgmod[tmpindex1],raterr_bkgmod[tmpindex1])
bkglc0=rate_bkgmod
tmpyf_bkg0 = GaussFilter(time_bkgmod,rate_bkgmod,100)
resi0 = rate_bkgmod - tmpyf_bkg0
tmpresi_bkg0 = GaussFilter(time_bkgmod,resi0,200)
rate_bkgmod = tmpyf_bkg0 + tmpresi_bkg0

'''
plt.figure()
plt.plot(time_bkgmod-time_bkgmod[0],rate_bkgmod)
plt.plot(time_bkgmod-time_bkgmod[0],bkglc0)
plt.plot(time_bkgmod-time_bkgmod[0],tmpyf_bkg0)
plt.show()
'''
...
if sp_lc_select == 'spec':
    ...
    tmpspec_err = bkgspec_allerr_bkgmap[0:bkgmap_num,3:1027]
    ...
    spec_cnt = spec_cnt * badpixel_cor
    #plt.figure()
    ...
    spec_list.close()
    outname = outnam +'.pha'
    #write_bkgspec(outname,spec_ch,spec_cnt,tmpexpo,spec_hdr)
    write_bkgspec2(outname,spec_ch,spec_cnt,spec_err,tmpexpo,spec_hdr)
    #plt.figure()
    #print("Expo=",tmpexpo)
    #plt.plot(rr)
    #plt.show()
...
if sp_lc_select == 'lc':
    ...
    #plt.figure()
    #plt.plot(time_bkgmod-time_bkgmod[0],rate_bkgmod)
    #plt.plot(lc_time-time_bkgmod[0],lc_counts)
    #plt.show()
    for ii in xrange(0,GTI_num):
        #print('ii=======================',ii)
        ...
        #print(tmptime_bkgmod[0]-time_bkgmod[0],np.max(tmptime_bkgmod-time_bkgmod[0]),np.size(tmptime_bkgmod))
    ...
    lc_bkg = lc_bkg * badpixel_cor
    lc_err = lc_err * badpixel_cor
    outname = outnam + '.lc'
    write_lcurve(outname,lc_time,lc_bkg,lc_err,lc_hdr)
    #write_lcurve(outname,lc_time,yf_bkg,lc_err,lc_hdr)
    '''For each det box'''
    outname = outnam + '_box0.lc'
    write_lcurve(outname,lc_time,lc_bkg*rr0_box0,lc_err*rr0_box0,lc_hdr)
    outname = outnam + '_box1.lc'
    write_lcurve(outname,lc_time,lc_bkg*rr0_box1,lc_err*rr0_box1,lc_hdr)
    outname = outnam + '_box2.lc'
    write_lcurve(outname,lc_time,lc_bkg*rr0_box2,lc_err*rr0_box2,lc_hdr)
    #print(lc_err*rr0_box0,lc_err*rr0_box1,lc_err*rr0_box2,'!!!')
    '''
    plt.figure()
    plt.plot(lc_time-lc_time[0],lc_bkg)
    plt.plot(lc_time-lc_time[0],yf_bkg)
    plt.show()
    '''
