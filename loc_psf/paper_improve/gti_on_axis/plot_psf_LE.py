...
def psf(Astr,paras):
    #paras = np.loadtxt('./Parabolic_%s_%s_%s.txt' % (instr, str(year),dealdate))
...
def deal_LE(data_dir,plot_dir='plot_psf'):
    energy_dir_list = glob.glob(data_dir+'/*keV')
    for i in range(len(energy_dir_list)):
        tree2_dirlist, tree2_tot = count_dir(energy_dir_list[i])
        energy = energy_dir_list[i][-6:]
        for j in range(tree2_tot):
            fits_dir = tree2_dirlist[j]
            print('fits_dir:',fits_dir)
            parabolic_txt_list = glob.glob(fits_dir+'/Parabolic*.txt')
            parabolic_txt = parabolic_txt_list[0]
            print('parabolic_txt_list:',parabolic_txt_list)
            year = parabolic_txt[-13:-9]
            dealdate = parabolic_txt[-8:-4]
            print(year,dealdate)
            Astr = 'A0{:s}'.format(str(int(year)-2017))
            paras = np.loadtxt(fits_dir+'/Parabolic_%s_%s_%s.txt' % (instr, str(year), dealdate))
            correct_psf = psf(Astr,paras)
            if not os.path.exists(plot_dir):
                os.mkdir(plot_dir)
            data_name = plot_dir+'/{:s}_{:s}_{:s}_psf'.format(instr,Astr,energy)
            npy_name = data_name + '.npy'
            if not os.path.exists(npy_name):
                np.save(data_name, correct_psf)
...
def plot_AA_delta(Z0,Z1,Astr0,Astr1,plot_dir='plot_psf'):
    fig = plt.figure(figsize=plt.figaspect(0.5))
    ax = fig.add_subplot(1, 1, 1)  # , projection='3d')
    ...
    # linewidth=0, antialiased=False)
    plt.colorbar(surf, orientation='horizontal')
    fig.savefig(plot_dir+'/delta_{:s}_{:s}.png'.format(Astr0,Astr1))
'''###
...
def plot_LE(plot_dir='plot_psf'):
    data_dir = './data'
    deal_LE(data_dir, plot_dir=plot_dir)
    old_para_list = ['/sharefs/hbkg/data/SCAN/PSF/psf_201904/Parabolic_LE_2017_201904.txt','/sharefs/hbkg/data/SCAN/PSF/psf_201904/Parabolic_LE_2018_201904.txt']
    for i,Astr in enumerate(['A01', 'A02']):
        paras = np.loadtxt(old_para_list[i])
        correct_psf = psf(Astr, paras)
        if not os.path.exists(plot_dir):
            os.mkdir(plot_dir)
        data_name = plot_dir + '/old{:s}_{:s}_psf'.format(instr, Astr)
        npy_name = data_name+'.npy'
        if not os.path.exists(npy_name):
            np.save(data_name, correct_psf)
        newpsf_list = glob.glob(plot_dir+'/*keV_psf.npy')
        for newpsf in newpsf_list:
            AstrLE_psf = np.load(npy_name)
            AstrLE_new_psf = np.load(newpsf)
            Astr1 = 'LEold'+Astr
            Astr2 = newpsf.split('/')[-1][:-4]
            print(Astr1,Astr2)
            plot_AA_delta(AstrLE_psf,AstrLE_new_psf,Astr1,Astr2)
