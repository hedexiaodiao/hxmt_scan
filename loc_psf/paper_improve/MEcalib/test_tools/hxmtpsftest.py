...
D_dec= []
D_ra0= []
#crt1,crt2,crt3=np.load('/hxmt/work/HXMT_scan_data/HE/lick_tools/psfcrt.txt')
#pol = [np.poly1d(crt3),np.poly1d(crt2),np.poly1d(crt1)]
polt = np.load('/hxmt/work/HXMT_scan_data/HE/lick_tools/poly_crt6.npy')
def crtf(crtalfa,crtbeta,crtroll):
    #if abs(crtalfa)<5.5:
    num = int(crtroll/60.+1)
    if crtroll == 0:
        dgr = int(crtbeta*6./2.2 + 3)
        #print ObsID,dgr,'in 0'
        pol = np.poly1d(polt[num][dgr])
        crtft = pol(crtalfa)
    else:
        dgr = int(crtbeta/0.275 + 4)
        #print ObsID,dgr,'in ',crtroll
        pol = np.poly1d(polt[num][dgr])
        crtft = pol(crtalfa)
    crtft = 0.5 if crtft<0.5 else crtft
    crtft = 2. if crtft>2. else crtft

    #dgr = int(crtbeta/0.55+2)
    #    pol = np.poly1d(polt[ObsID][dgr])
    #    crtft = pol(crtalfa)
    #    crtft = 0.5 if crtft<0.5 else crtft
    #    crtft = 2. if crtft>2. else crtft
    #else:
    #    crtft = 1.
    #print crtroll,dgr,ObsID,crtft
    return crtft
...
def hxmtpsf(engs, parameter, flux):
    ...
    for s_step in range(0,Nflux): 
        ...
        if delta_ra<ra_bound and delta_dec<dec_bound: 
            ...
            (1.0-tan(delta_dec/180.0*PI)/tan(dec_bound/180.0*PI))*crtf(ra_crt0,dec_crt,parameter[1])
        else:
            factor=0
