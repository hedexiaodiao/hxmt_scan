...
print ("the scanning pointing file : ",infile,instr)
########################################################
scnid = sys.argv[2]
#scnid='ALL'
PolyCrt = np.load("/sharefs/hbkg/data/SCAN/PSF/psf_factor/%s%s_polycrt.npy"%(instr.split()[0],scnid)).tolist()
...
def crt_f(crtalfa0,crtbeta0,crtalfa,crtbeta,crtroll):
    num = int(crtroll/60. + 1)
    if crtroll == 0:
        dgr = int(crtbeta*26./8. + 13)
        pol = np.poly1d(PolyCrt[num][dgr])
        crtft = pol(crtalfa0)
    else:
        dgr = int(crtalfa*3. + 3)
        pol = np.poly1d(PolyCrt[num][dgr])
...
def hxmtpsf(engs, parameter, flux):
    ...
    for s_step in range(0,Nflux): 
        ...
        if delta_ra<ra_bound and delta_dec<dec_bound: 
            ...
            (1.0-tan(delta_dec/180.0*PI)/tan(dec_bound/180.0*PI))*crt_f(ra_crt0,dec_crt0,ra_crt,dec_crt,parameter[1])
        else:
            factor=0
