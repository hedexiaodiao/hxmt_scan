...
for select_box_dex in range(3):
    ...
    lcdt = lchd[1].data
    qtime = atdt.field(0)
    lctime = 259268965.131675+np.arange(3005,3016+1)###lcdt.field(0)
    tstart = lctime[0]
    tstop = lctime[-1]
    ###print('check file',qtime[0:4],lctime[0],atdt.field(1))
    # ----------------quat with lc time------------#
    q1_f = interpolate.interp1d(qtime,atdt.field(1),kind='linear')
    q1_list = q1_f(lctime)
    q2_f = interpolate.interp1d(qtime, atdt.field(2), kind='linear')
    q2_list = q2_f(lctime)
    q3_f = interpolate.interp1d(qtime, atdt.field(3), kind='linear')
    q3_list = q3_f(lctime)
    # q1_f = interpolate.interp1d(x, y, kind='linear')
    # q2_list = atdt.field(2)[np.in1d(qtime, lctime)]
    # q3_list = atdt.field(3)[np.in1d(qtime, lctime)]
    ###print(q1_list,q2_list,q3_list)
    ...
    for i in range(0,q1_list.shape[0]):
        ...
        ###print('i',i,delta_alfa_box,delta_beta_box)
        ...
        accept_alpha.append(delta_alfa_box)
        accept_beta.append(delta_beta_box)
    print('%smean:'%instr,select_box_dex,np.mean(accept_alpha),np.mean(accept_beta))
    #-------------------用以多段好时间的筛选-----------------
