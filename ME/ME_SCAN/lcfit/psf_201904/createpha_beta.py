...
if __name__ == "__main__":
    ...
    for obid in lclist:
        if os.path.exists('%s/%s/me_3box2.grp' % (ProdWpath, obid)):
            continue
        if not os.path.exists('%s/%s/me_lc_box2_small_cut.fits' % (NetWpath, obid)):
            continue
        os.system("mkdir %s/pfiles_%s_load" % (Ppath, obid))
        os.environ[
            "PFILES"] = "%s/pfiles_%s_load;/home/hxmt/hxmtsoft2/soft/install/x86_64-unknown-linux-gnu-libc2.12/syspfiles" % (
        Ppath, obid)
        for i in range(3):
            patha = NetWpath + '/' + obid + '/me_lc_box%s_small_cut.fits' % i
            pathb = ProdWpath + '/' + obid + '/me_3box%s' % i
            os.chdir(ProdWpath + '/'+obid)
            cpha(patha, pathb)
