...
if instr == "HE":
    ra_bound=7.125#5.7 #means long_bound
    dec_bound=1.375#1.1 #means short_bound
elif instr == "ME":
...
ratio = np.load('/sharefs/hbkg/data/SCAN/PSF/psf_201904/FactorsGrid_2018_%s.npy'%instr)
seg_af,seg_bt = 50,50
D_ra = []
D_dec= []
def hxmtpsf(engs, parameter, flux):
    ...
    for s_step in range(0,Nflux): 
        ...
        alfa_tp = delta_ra0*cos(roll)-delta_dec0*sin(roll)
        beta_tp = delta_ra0*sin(roll)+delta_dec0*cos(roll)
        ...
        if delta_ra<ra_bound and delta_dec<dec_bound:
           # print 'GO on::',
            ratio1 = int(np.floor(seg_af*(alfa_tp+ra_bound)/ra_bound/2.))
            ratio2 = int(np.floor(seg_bt*(beta_tp+dec_bound)/dec_bound/2.))
            rat_indx = ratio1 * seg_bt + ratio2
            #print ratio1,ratio2,rat_indx
            factor = ratio[roll_indx][rat_indx]
            #factor=r*(1.0-tan(delta_ra/180.0*PI)/tan(ra_bound/180.0*PI))*\
